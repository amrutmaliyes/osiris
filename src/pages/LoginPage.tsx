...
function LoginPage({ onLoginSuccess }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const loginSuccessful = await invoke('perform_login', { credentials: { email, password } as LoginCredentials }) as boolean;
    ...
    <div className="container mx-auto mt-8 text-center">
      <div className="p-4 bg-white rounded-md shadow-md">
        ...
        <div className="mt-4 text-sm">
          <Link to="#" onClick={() => alert("Forgot Password functionality not implemented yet.")} className="text-blue-600 hover:underline">
            Forgot Password
          </Link>
        </div>
        {error && (
          <div className="mt-4 p-3 rounded bg-red-500 text-white">
            {error}
          </div>
        )}
      </div>
